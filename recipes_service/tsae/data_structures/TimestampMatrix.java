...
public class TimestampMatrix implements Serializable{
	...
	public void updateMax(TimestampMatrix tsMatrix){
		for(Map.Entry<String, TimestampVector> entry : tsMatrix.timestampMatrix.entrySet()) {
			...
			TimestampVector receivedValue = entry.getValue();
			TimestampVector ownValue = timestampMatrix.get(key);
			if(ownValue!=null) ownValue.updateMax(receivedValue);
		}
	}
	...
	public TimestampVector minTimestampVector(){
		...
		for(TimestampVector tv: timestampMatrix.values()) {
			if(result == null)
				result = tv.clone();
			else
				result.mergeMin(tv);
	...
	public TimestampMatrix clone(){
		...
		TimestampMatrix result = new TimestampMatrix((List<String>) timestampMatrix.keys());
		
		for(Map.Entry<String, TimestampVector> entry: timestampMatrix.entrySet()) {
			result.timestampMatrix.put(entry.getKey(), entry.getValue());
		}
